(function () {
  ...
  function renderBlocks(blocks) {
    return (blocks || []).map((b) => {
      ...
      if (b.type === 'image') return `<figure class="nl-block polaroid nl-photo" style="--rot:-1.5deg"><img src="${escape(b.src)}" alt="${escape(b.caption || 'Newsletter photo')}" /><figcaption>${escape(b.caption || '')}</figcaption></figure>`;
      if (b.type === 'timeline') {
    ...
    try {
      ...
      archive.innerHTML = data.newsletters.map((edition, index) =>
        `<a class="nl-card" href="?edition=${edition.id}" data-id="${edition.id}">
          <figure class="nl-edition-polaroid" style="--rot:${index % 2 ? 2 : -2}deg"><img src="/assets/gallery/gallery-${(index % 19) + 1}.png" alt="" /></figure>
