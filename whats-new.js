(function () {
  ...
  function createGuide({ id, storageKey, slides, waitForChecklist }) {
    ...
    modal.innerHTML = `
      <div class="whats-new-frame">
        ...
        <div class="modal-sheet modal-card modal-card-wide whats-new-sheet" role="dialog" aria-modal="true" aria-labelledby="${id}Title">
          ...
          <button class="whats-new-never" type="button" data-whats-never>Never show this again</button>
        </div>
    ...
    function render() {
      ...
      const action = slide.outreach
        ...
        : slide.href
          ? `<a class="button button-line" href="${slide.href}">${slide.action}</a>`
          : '';
      stage.innerHTML = `
        <article class="whats-new-card">
          <span class="eyebrow">${slide.eyebrow}</span>
          <h2 id="${id}Title">${slide.title}</h2>
          <p>${slide.copy}</p>
          ${action}
        </article>`;
    ...
    function open() {
      if (opened || modal.classList.contains('open') || localStorage.getItem(storageKey) === '1') return;
      opened = true;
      index = 0;
      render();
    ...
    modal.querySelector('[data-whats-never]').addEventListener('click', neverShow);
